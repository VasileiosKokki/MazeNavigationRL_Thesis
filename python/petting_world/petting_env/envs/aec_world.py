...
class CustomActionMaskedEnvironment(AECEnv):
    ...
    def __init__(self, render_mode=None):
        ...
        self.timestep = None
        self.possible_agents = ["prisoner", "guard1", "guard2"]
    ...
    def step_all(self, actions):
        # print(actions)
        if "prisoner" in actions and "guard1" in actions and "guard2" in actions:
        ...
        new_distance1 = abs(new_guard1_location[0] - new_prisoner_location[0]) + abs(new_guard1_location[1] - new_prisoner_location[1])
        new_distance2 = abs(new_guard2_location[0] - new_prisoner_location[0]) + abs(new_guard2_location[1] - new_prisoner_location[1])
        ...
        if np.array_equal(new_prisoner_location, new_guard1_location) or np.array_equal(new_prisoner_location, new_guard2_location) \
            or (np.array_equal(new_guard1_location, self.prisoner_location) and np.array_equal(new_prisoner_location, self.guard1_location)) \
                or (np.array_equal(new_guard2_location, self.prisoner_location) and np.array_equal(new_prisoner_location, self.guard2_location)):
            prisoner_reward = -1
            guard1_reward = 1
            guard2_reward = 1
        ...
        rewards = {"prisoner": prisoner_reward, "guard1": guard1_reward, "guard2": guard2_reward}
        # Get observations
    ...
    def get_obs(self):
        observation = {
            "prisoner": np.array((self.prisoner_location, self.guard1_location, self.guard2_location, np.array([0,0]))),
